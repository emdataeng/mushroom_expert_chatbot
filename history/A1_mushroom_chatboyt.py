...
def extract_json_and_explanation(output_text: str):
    ...
    #for debugging print json_part and explanation, preceded by the name of the current function
    log_troubleshooting(f"extract_json_and_explanation - json_part: {json_part}")
    log_troubleshooting(f"extract_json_and_explanation - explanation: {explanation}")
...
def call_model(user_prompt, image, temp, topK, topP, keep_in_mind=None) -> str:
    ...
    #for debugging we log the output_text preceded by the name of the current function
    log_troubleshooting(f"call_model - output_text: {output_text}")
...
def chatbot_interface(user_prompt, image=None, temp=0.7, topK=0, topP=0.5, image_filename=None):
    ...
    #for debugging, we log the current memory and the extracted json and explanation, preceded by the name of the current function
    log_troubleshooting(f"chatbot_interface - function call")
    log_troubleshooting(f"Keep in mind: {chatbot_interface.keep_in_mind}")
    log_troubleshooting(f"expert_reply_json: {expert_reply_json}")        # The structured JSON block
    log_troubleshooting(f"expert_reply_explanation: {expert_reply_explanation}") # The natural-language explanation
    log_troubleshooting(f"time_taken: {time_taken}")
    #####
